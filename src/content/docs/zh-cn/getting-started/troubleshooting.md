...
1. 提供者标签页顶部的**检查设置**测试每项工作并指出失败的提供者。
2. 提供者表单上的**测试提供者**显示服务器返回的具体错误。
3. **Twinny** 输出通道（**Twinny - Show logs**，或状态栏菜单）每个请求一行，包含模型、耗时和停止原因。把通道日志级别设为 *Debug* 可看到完整提示和回复。API 密钥会被脱敏。
