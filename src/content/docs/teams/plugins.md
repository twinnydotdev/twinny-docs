...
A finished review can be posted back to the host as a comment, a change request or an approval, with a footer naming the model; **auto-post** does that for every review without a click. Tick **auto-review** on a repository and its new and updated pulls are reviewed in the background: one at a time, newest first, never drafts, and only while no developer request is running, so completions and chats are never slowed down. Reviews are routed like any other request and appear in usage under `plugin:github` or `plugin:gitlab`, so the Usage page shows what they cost.
